...
class Model():
    ...
    def predict(self, onesample: np.ndarray):
        # onesample是一个样本，一个矩阵
        row_data = []
        for each_flow in onesample:
            row_data.extend(each_flow)
        ...
        print(f"预测正确的个数:{cnt_predict_right}")
        print(f"预测错误的个数:{cnt_predict_wrong}")
    # print(df)
...
if __name__ == "__main__":
    ...
    model = KMeans(n_clusters=7,random_state=RANDOMSEED+1)
    ...
    MAX_PACKETS = 15
    allsamples = []
    for black_bag in tqdm(lst_black_bags[:]):
        feats_flows, label = packetFeats.get_features_from_onebag(black_bag.lst_flows)
        ...
        for each_flow in feats_flows:
            ...
            # 聚类
            output = list(model.predict(scaler_np_each_flow))
    ...
    # 建bayesnet
    base_edges = construct_base_edges(MAX_FLOWS, MAX_PACKETS)
    model = BayesianNetwork(base_edges) # input: 边集合  output: model
    # model.fit(data, estimator=MaximumLikelihoodEstimator) # 最大似然学习参数

    # 创建一个空的 DataFrame
    columns = create_columns(MAX_FLOWS, MAX_PACKETS)
    df = pd.DataFrame(columns=columns)
    print(df.head())
    # 为每个节点添加数据
    index = 0
    for onesample in tqdm(allsamples[:]):
        row_data = [] # 一行数据
        for each_flow in onesample:
            row_data.extend(each_flow)
        print(row_data)
        if len(row_data)>MAX_FLOWS*MAX_PACKETS:
            row_data = row_data[:MAX_FLOWS*MAX_PACKETS]
        print(len(row_data))
        df.loc[index] = row_data
        index += 1
    print(df.head())
    model.fit(df, estimator=MaximumLikelihoodEstimator) # 最大似然学习参数
    # 使用 BeliefPropagation 进行推断
    from pgmpy.inference import BeliefPropagation
    inference_bp = BeliefPropagation(model)
    query_bp = inference_bp.map_query(variables=['Packet13'], evidence={'Packet11': 0,'Packet12': 2})
    print("信念传播算法的推断结果：", query_bp)
    # print(df)